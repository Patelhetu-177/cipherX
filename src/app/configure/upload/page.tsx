...
import {
  ...
  FileText,
  FileJson,
} from "lucide-react"; // Import icons for PDF, CSV, JSON
...
const Page = () => {
  ...
  const { startUpload: startImageUpload, isUploading: isImageUploading } =
    useUploadThing("imageUploader", {
      onClientUploadComplete: ([data]) => {
        ...
        startTransition(() => {
          router.push(`/configure/analysis?id=${configId}`);
        });
      },
      onUploadProgress(p) {
        setUploadProgress(p);
  ...
  const { startUpload: startCsvUpload, isUploading: isCsvUploading } =
    useUploadThing("csvUploader", {
      onClientUploadComplete: ([data]) => {
        const configId = data.serverData.configId;
        startTransition(() => {
          router.push(`/configure/analysis?id=${configId}`);
  ...
  const { startUpload: startJsonUpload, isUploading: isJsonUploading } =
    useUploadThing("JSONUploader", {
      onClientUploadComplete: ([data]) => {
        const configId = data.serverData.configId;
        startTransition(() => {
          router.push(`/configure/analysis?id=${configId}`);
        ...
        >
          {({ getRootProps, getInputProps }) => (
            ...
            >
              ...
              {isPending ? null : (
                <p className="text-xs text-zinc-500">PDF, CSV, JSON</p>
              )}
            </div>
          )}
        </Dropzone>
